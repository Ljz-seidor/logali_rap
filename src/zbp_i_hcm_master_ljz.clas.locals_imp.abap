CLASS lcl_buffer DEFINITION.
  PUBLIC SECTION.
    CONSTANTS: c_created TYPE c LENGTH 1 VALUE 'C',
               c_updated TYPE c LENGTH 1 VALUE 'U',
               c_delated TYPE c LENGTH 1 VALUE 'D'.


    TYPES: BEGIN OF ty_buffer_master.
             INCLUDE TYPE zhcm_master_ljz AS data.
...
CLASS lhc_HCMMaster IMPLEMENTATION.
  ...
  METHOD create.
    ...
    SELECT MAX( e_number ) FROM zhcm_master_ljz
    ...
    LOOP AT entities INTO DATA(ls_entities).
      ...
      "Se mueve la informacion a un BUFFER para que no se pierda la información entre el metodo y el metodo sava
      "INSERT XQ es SORTED --> no se puede hacer APPEND
      INSERT VALUE #( flag = lcl_buffer=>c_created
      ...
      IF NOT ls_entities-%cid IS INITIAL.
        INSERT VALUE #( %cid = ls_entities-%cid
                ...
                INTO TABLE mapped-hcmmaster. "Notificamos al standar que ya usamos el cid y e_number
      ENDIF.
  ...
  METHOD update.
    ...
    LOOP AT entities INTO DATA(ls_entities).
      ...
      SELECT SINGLE * FROM zhcm_master_ljz
      ...
      "Se trabaja con la estructura %control xq el framework nos indica si se ha modificado o no
      INSERT VALUE #( flag = lcl_buffer=>c_updated
                      data = VALUE #( e_number = ls_entities-%data-e_number
                                      e_name = COND #( WHEN ls_entities-%control-e_name EQ if_abap_behv=>mk-on
                                                          THEN ls_entities-%data-e_name "se toma la modificacion
                                                        ELSE ls_ddbb-e_name )  "se toma de la bd
  ...
  METHOD delete.
      ...
      INSERT VALUE #( flag = lcl_buffer=>c_delated
...
CLASS lsc_Z_I_HCM_MASTER_LJZ DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_Z_I_HCM_MASTER_LJZ IMPLEMENTATION.

  METHOD finalize.
    "Despues del guardado de los datos
    """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
  ENDMETHOD.
  ...
  METHOD save.
    ...
    DATA: lt_data_created TYPE STANDARD TABLE OF zhcm_master_ljz,
          lt_data_updated TYPE STANDARD TABLE OF zhcm_master_ljz,
          lt_data_deleted TYPE STANDARD TABLE OF zhcm_master_ljz.

    lt_data_created = VALUE #( FOR <row> IN lcl_buffer=>mt_buffer_master
                                         WHERE ( flag = lcl_buffer=>c_created )
                      ( <row>-data ) ).

    IF NOT lt_data_created IS INITIAL.
      INSERT zhcm_master_ljz FROM TABLE @lt_data_created.
    ENDIF.

    lt_data_updated = VALUE #( FOR <row> IN lcl_buffer=>mt_buffer_master
                                         WHERE ( flag = lcl_buffer=>c_updated )
                      ( <row>-data ) ).

    IF NOT lt_data_updated IS INITIAL.
      UPDATE zhcm_master_ljz FROM TABLE @lt_data_updated.
    ENDIF.

    lt_data_deleted = VALUE #( FOR <row> IN lcl_buffer=>mt_buffer_master
                                         WHERE ( flag = lcl_buffer=>c_delated )
                      ( <row>-data ) ).

    IF NOT lt_data_deleted IS INITIAL.
      DELETE zhcm_master_ljz FROM TABLE @lt_data_deleted.
    ENDIF.
